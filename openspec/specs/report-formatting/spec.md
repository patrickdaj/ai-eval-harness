...
Define how the eval harness renders its run report — a rich, color-coded terminal
presentation of the run summary and failures when supported, with a faithful plain-text
fallback — while preserving the harness's existing metrics and exit-code behavior.
